...
$oui='oui';
    ...
    $requete = 'SELECT COUNT(*) From fiche where unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat = $connection->prepare($requete);
    $resultat->execute();
    $nbre_mal = $resultat->fetchColumn();
    

    $requete1 = 'SELECT COUNT(test_p) From fiche WHERE test_p = \'oui\' and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat1 = $connection->prepare($requete1);
    $resultat1->execute();
    $nbre_test = $resultat1->fetchColumn();

    $requete2 = 'SELECT COUNT(retrait_p) From fiche WHERE retrait_p = \'oui\' and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat2 = $connection->prepare($requete2);
    $resultat2->execute();
    $nbre_retrait = $resultat2->fetchColumn();

    $requete3 = 'SELECT COUNT(positif_p) From fiche WHERE positif_p = \'oui\' and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat3 = $connection->prepare($requete3);
    $resultat3->execute();
    $nbre_positif = $resultat3->fetchColumn();

    $requete4 = 'SELECT COUNT(contact_p) From fiche WHERE contact_p = \'oui\' and unite_f=\''. $_SESSION['unite_c'] .'\'';
    ...
    $requete10 = 'SELECT COUNT(sousarv_p) From fiche WHERE sousarv_p=\'oui\' and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat10 = $connection->prepare($requete10);
    $resultat10->execute();
    $nbre_arv = $resultat10->fetchColumn();

    $requete11 = 'SELECT COUNT(etat_p) From fiche WHERE etat_p="hospitalisation" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat11 = $connection->prepare($requete11);
    $resultat11->execute();
    $nbre_hosp = $resultat11->fetchColumn();

    $requete12 = 'SELECT COUNT(etat_p) From fiche WHERE etat_p="perdu" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat12 = $connection->prepare($requete12);
    $resultat12->execute();
    $nbre_perd = $resultat12->fetchColumn();

    $requete13 = 'SELECT COUNT(etat_p) From fiche WHERE etat_p="deced" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat13 = $connection->prepare($requete13);
    $resultat13->execute();
    $nbre_dece = $resultat13->fetchColumn();

    $requete14 = 'SELECT COUNT(etat_p) From fiche WHERE etat_p="recup" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat14 = $connection->prepare($requete14);
    $resultat14->execute();
    $nbre_recup = $resultat14->fetchColumn();

    $requete15 = 'SELECT COUNT(etat_p) From fiche WHERE etat_p="aucun" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat15 = $connection->prepare($requete15);
    $resultat15->execute();
    $nbre_aucun = $resultat15->fetchColumn();

    $requete16 = 'SELECT COUNT(rithme_p) From fiche WHERE rithme_p="jour" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat16 = $connection->prepare($requete16);
    $resultat16->execute();
    $nbre_jour = $resultat16->fetchColumn();

    $requete17 = 'SELECT COUNT(rithme_p) From fiche WHERE rithme_p="aucun" and unite_f=\''. $_SESSION['unite_c'] .'\'';
    $resultat17 = $connection->prepare($requete17);
    $resultat17->execute();
    $nbre_pasjour = $resultat17->fetchColumn();

    $requete18 = 'SELECT SUM(sensibilisation) From unite where nom_u=\''. $_SESSION['unite_c'] .'\'';
    $resultat18 = $connection->prepare($requete18);
    $resultat18->execute();
    $nbre_cumsens = $resultat18->fetchColumn();

    $requete19 = 'SELECT SUM(distributions) From unite where nom_u=\''. $_SESSION['unite_c'] .'\'';
    $resultat19 = $connection->prepare($requete19);
    $resultat19->execute();
    $nbre_cumdistr = $resultat19->fetchColumn();

    $requete20 = 'SELECT SUM(ist_consult) From unite where nom_u=\''. $_SESSION['unite_c'] .'\'';
    $resultat20 = $connection->prepare($requete20);
    $resultat20->execute();
    $nbre_cumcons = $resultat20->fetchColumn();

    $requete21 = 'SELECT SUM(ist_diag) From unite where nom_u=\''. $_SESSION['unite_c'] .'\'';
    $resultat21 = $connection->prepare($requete21);
    $resultat21->execute();
    $nbre_cumdiag = $resultat21->fetchColumn();

    $requete22 = 'SELECT SUM(ist_contact) From unite where nom_u=\''. $_SESSION['unite_c'] .'\'';
    $resultat22 = $connection->prepare($requete22);
    $resultat22->execute();
    $nbre_cumcont = $resultat22->fetchColumn();

if(isset($_POST['fichesub']))
{
    $ageca = htmlspecialchars($_POST['ageca']);
    $agec = htmlspecialchars($_POST['agec']);
    $age = htmlspecialchars($_POST['age']);
    $ag = htmlspecialchars($_POST['ag']);
    $ag1 = htmlspecialchars($_POST['ag1']);
    $jour = htmlspecialchars($_POST['jour']);

    $reque1 = "INSERT INTO unite(zone_u, aire_u, nom_u, sensibilisation, distributions, ist_consult, ist_diag, ist_contact, date_ajout) VALUES(?,?,?,?,?,?,?,?,?)";
    $insert1 = $connection->prepare($reque1);
    $insert1->execute(array($_SESSION['zone_c'], $_SESSION['aire_c'], $_SESSION['unite_c'], $ageca, $agec, $age, $ag, $ag1, $jour));
}
?>
<html>
    <head>
    ...
    <body>
        ...
        <div align="center">
            <section class="insc" style="width: 800px;height: 1300px;">
                <h3><strong class="first">Données de l'unité</strong></h3>
                <br/><br/>
                            ...
                            <div class="tete">
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td align="center"><label for="zone"><?php echo $_SESSION['aire_c']; ?></label> </td>  
                                        <td align="center"><label for="zone"><?php echo $_SESSION['unite_c']; ?></label> </td>
                                    </tr>
                                </table>
                            </div>
                    </fieldset>
                    <fieldset id="pet">
                        <legend><strong style="font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif;font-size: 1.3em;">Informations spécifique</strong></legend>
                        <table>
                            <tr> 
                        ...
                        <table>    
                            <tr>
                                <td></td>
                                <td ><strong class="sec"> Informations additionnelles</td>
                            ...
                            <tr>                                      
                                ...
                                <td><input type="number" id="ageca" name="ageca" value="" class="num"/></td>
                            ...
                            <tr>
                                ...
                                <td><input type="number" id="ageca" name="agec" value="" class="num"/></td>
                            ...
                            <tr>
                                ...
                                <td><input type="number" id="age" name="age" value="" class="num"/></td>
                            ...
                            <tr>
                                ...
                                <td><input type="number" id="ag" name="ag" value="" class="num"/></td>
                            ...
                            <tr>
                                ...
                                <td><input type="number" id="ag1" name="ag1" value="" class="num"/></td>
                                <td><label for="cumules" class="tdd"><?php echo $nbre_cumcont;?></label></label></td>
                            </tr>
                        </table>
                        <br/>
                        <label for="naissance"><strong class="second">Date d'ajout :</strong></label><input type="date" name="jour"/><br/>     
                    </fieldset>
                    <div class="fieldsets">
                        <input type="submit" name="fichesub" value="Enregistrer" align="center"/>
